    ...
    this.connectToSocket = function (wsUrl) {
        ...
        connection.onmessage = function (message) {
            ...
            switch (data.SignalType) {
                ...
                case 's2dp':
                    if (data.From != myNo) {
                        var desc = new RTCSessionDescription(data.Raw);
                ...
                case 'iceCandidates':
                    if (data.From != myNo) {
                        console.log("got ice candidate - adding");
                        remoteIceCandidates.push(data.Raw);
                        if (isRemoteSet) {
                            addIceCandidates();
                        }
                    }
                    break;
                case 'text':
                    $(msgWindow).append("<div id='text11'>" + getFormattedDate() + "] " + data.Raw + "</div>");
                    break;
                case 'question':
                    $(msgWindow).append("<div id='text11'>" + getFormattedDate() + "] " + data.Raw + "</div>");
                    break;
