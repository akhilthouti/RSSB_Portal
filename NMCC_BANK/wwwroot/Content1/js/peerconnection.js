...
const recordbutton = document.getElementById(meetingOptions.record);  //not in original js
...
function gotRemoteStream(e) {
    ...
    hangupButton.disabled = false;
	 recordbutton.disabled = false;  //not in original js
}
...
function uploadSnapshot() {
    const blob = createBlob(snapshotcanvas.toDataURL("image/jpeg"));
    var fd = new FormData();
    fd.append('data', blob);

    $.ajax({
        url: baseurl + "/api/meeting/" + meetingOptions.meetingId + "/uploadimage",
        type: 'POST',
        data: fd,
        processData: false,
        contentType: false
    }).done(function (data) {
        meetingOptions.uploadSnapshotCompleteEvent();
    });
}
